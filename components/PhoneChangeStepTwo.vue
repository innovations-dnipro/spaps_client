<template>
  <div class="s-login-description">
    {{ $t('phone_change_messages.new_phone_number_confirmation') }}
  </div>
...
const onContinueButtonClick = async () => {
  ...
  try {
    const [response] = await asyncGlobalSpinner(
      api.AuthorizationService.confirmPhoneChangeCode({
    ...
    if (response?.phone) {
      toast.success(
        $i18n.t('phone_change_messages.your_phone_number_was_changed')
      );
